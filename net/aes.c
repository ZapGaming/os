#include <net/aes.h>
#include <string.h>

/* AES (FIPS-197) forward S-box. */
static const uint8_t sbox[256] = {
    
};

/* Round constants for the key schedule (Rcon[i] is used for word i/4). */
static const uint8_t rcon[10] = { 0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1b,0x36 };

static inline uint8_t xtime(uint8_t x) {
    /* Multiply by {02} in GF(2^8) with the AES reduction polynomial. */
    return (uint8_t)((x << 1) ^ ((x & 0x80) ? 0x1b : 0x00));
}

/* Multiply two bytes in GF(2^8) (used by MixColumns). */
static uint8_t gmul(uint8_t a, uint8_t b) {
    uint8_t p = 0;
    for (int i = 0; i < 8; i++) {
        if (b & 1) p ^= a;
        a = xtime(a);
        b >>= 1;
    }
    return p;
}

static inline uint32_t sub_word(uint32_t w) {
    uint8_t b0 = sbox[(w >> 24) & 0xFF];
    uint8_t b1 = sbox[(w >> 16) & 0xFF];
    uint8_t b2 = sbox[(w >> 8) & 0xFF];
    uint8_t b3 = sbox[w & 0xFF];
    return ((uint32_t)b0 << 24) | ((uint32_t)b1 << 16) | ((uint32_t)b2 << 8) | b3;
}

static inline uint32_t rot_word(uint32_t w) {
    /* [a0,a1,a2,a3] -> [a1,a2,a3,a0], bytes packed MSB-first as above. */
    return (w << 8) | (w >> 24);
}

void aes128_init(aes128_ctx *ctx, const uint8_t key[AES128_KEY_SIZE]) {
    uint32_t w[44]; /* Nb*(Nr+1) = 4*11 = 44 words */

    for (int i = 0; i < 4; i++) {
        w[i] = ((uint32_t)key[4 * i] << 24) | ((uint32_t)key[4 * i + 1] << 16) |
               ((uint32_t)key[4 * i + 2] << 8) | (uint32_t)key[4 * i + 3];
    }

    for (int i = 4; i < 44; i++) {
        uint32_t temp = w[i - 1];
        if (i % 4 == 0) {
            temp = sub_word(rot_word(temp)) ^ ((uint32_t)rcon[i / 4 - 1] << 24);
        }
        w[i] = w[i - 4] ^ temp;
    }

    for (int round = 0; round <= AES128_ROUNDS; round++) {
        for (int col = 0; col < 4; col++) {
            ctx->round_key[round][col] = w[round * 4 + col];
        }
    }
}

/* state is column-major per FIPS-197: byte (row r, col c) lives at
 * state[r + 4*c]. */

static void add_round_key(uint8_t state[16], const uint32_t rk[4]) {
    for (int c = 0; c < 4; c++) {
        state[4 * c]     ^= (uint8_t)(rk[c] >> 24);
        state[4 * c + 1] ^= (uint8_t)(rk[c] >> 16);
        state[4 * c + 2] ^= (uint8_t)(rk[c] >> 8);
        state[4 * c + 3] ^= (uint8_t)(rk[c]);
    }
}

static void sub_bytes(uint8_t state[16]) {
    for (int i = 0; i < 16; i++) state[i] = sbox[state[i]];
}

static void shift_rows(uint8_t state[16]) {
    uint8_t t[16];
    memcpy(t, state, 16);
    /* Row r is cyclically left-shifted by r columns. */
    for (int r = 0; r < 4; r++) {
        for (int c = 0; c < 4; c++) {
            state[r + 4 * c] = t[r + 4 * ((c + r) % 4)];
        }
    }
}

static void mix_columns(uint8_t state[16]) {
    for (int c = 0; c < 4; c++) {
        uint8_t a0 = state[4 * c], a1 = state[4 * c + 1], a2 = state[4 * c + 2], a3 = state[4 * c + 3];
        state[4 * c]     = (uint8_t)(gmul(a0, 2) ^ gmul(a1, 3) ^ a2 ^ a3);
        state[4 * c + 1] = (uint8_t)(a0 ^ gmul(a1, 2) ^ gmul(a2, 3) ^ a3);
        state[4 * c + 2] = (uint8_t)(a0 ^ a1 ^ gmul(a2, 2) ^ gmul(a3, 3));
        state[4 * c + 3] = (uint8_t)(gmul(a0, 3) ^ a1 ^ a2 ^ gmul(a3, 2));
    }
}

void aes128_encrypt_block(const aes128_ctx *ctx, const uint8_t in[AES128_BLOCK_SIZE],
                           uint8_t out[AES128_BLOCK_SIZE]) {
    uint8_t state[16];
    memcpy(state, in, 16);

    add_round_key(state, ctx->round_key[0]);
    for (int round = 1; round < AES128_ROUNDS; round++) {
        sub_bytes(state);
        shift_rows(state);
        mix_columns(state);
        add_round_key(state, ctx->round_key[round]);
    }
    sub_bytes(state);
    shift_rows(state);
    add_round_key(state, ctx->round_key[AES128_ROUNDS]);

    memcpy(out, state, 16);
}
