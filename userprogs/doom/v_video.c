...
/* ZapOS has no mouse support (usemouse is always 0), and the original
 * body divided by mouse_acceleration (a float) to undo mouse
 * acceleration for display -- this build has no FPU at all
 * (-mgeneral-regs-only), so that arithmetic can't exist anywhere in
 * the compiled output, reachable or not. Nothing ever needs this
 * debug widget without a mouse, so it's just a stub. */
